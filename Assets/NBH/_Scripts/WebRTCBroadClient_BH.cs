...
public class WebRTCBroadClient_BH : MonoBehaviourPun, IOnPhotonViewOwnerChange
{
    ...
    public void Connect()
    {
        ...
        {
            StartCoroutine(CreateOfferInit());
        }
    }
    ...
    RTCPeerConnection CreateLocalPeer()
    {
        ...
        pc.OnConnectionStateChange = state =>
        {
            if (printDebug)
            {
                Debug.Log($"1 {GetInstanceID()} OnConnectionStateChange: {state}");
            }
        };

        pc.OnNegotiationNeeded = () =>
        {
            StartCoroutine(CreateOffer(pc, photonView.Controller));
        };
    ...
    RTCPeerConnection CreateRemotePeer(Photon.Realtime.Player fromPlayer)
    {
        ...
        pc.OnNegotiationNeeded = () =>
        {
            StartCoroutine(CreateOffer(pc, fromPlayer));
        };
    ...
    public IEnumerator CreateOfferInit()
    {
        Debug.LogAssertion($"CreateOfferInit controller: {photonView.Controller}");
    ...
    public IEnumerator CreateOffer(RTCPeerConnection pc, Photon.Realtime.Player toPlayer)
    {
    ...
    public void ReceiveAnswer(string desc, Photon.Realtime.Player fromPlayer)
    {
        if(printDebug)
        {
            print($"ReceiveAnswer{fromPlayer}\n {desc}");
    ...
    private void OnSetRemoteSuccess(RTCPeerConnection pc)
    {
        if (printDebug)
        {
            Debug.Log($"{GetInstanceID()} SetRemoteDescription comoplete");
        }
    }
    ...
    private void OnIceConnectionChange(RTCPeerConnection pc, RTCIceConnectionState state)
    {
        switch(state)
        {
            case RTCIceConnectionState.New:
                if (printDebug)
                {
                    Debug.Log($"{GetInstanceID()} IceConnectionState: New");
                }
                break;
            case RTCIceConnectionState.Checking:
                if (printDebug)
                {
                    Debug.Log($"{GetInstanceID()} IceConnectionState: Checking");
                }
                break;
            case RTCIceConnectionState.Closed:
                if (printDebug)
                {
                    Debug.Log($"{GetInstanceID()} IceConnectionState: Closed");
                }
                break;
            case RTCIceConnectionState.Completed:
                if (printDebug)
                {
                    Debug.Log($"{GetInstanceID()} IceConnectionState: Completed");
                }
                break;
            case RTCIceConnectionState.Connected:
                if (printDebug)
                {
                    Debug.Log($"{GetInstanceID()} IceConnectionState: Connected");
                }
                break;
            case RTCIceConnectionState.Disconnected:
                if (printDebug)
                {
                    Debug.Log($"{GetInstanceID()} IceConnectionState: Disconnected");
                }
                break;
            case RTCIceConnectionState.Failed:
                if (printDebug)
                {
                    Debug.Log($"{GetInstanceID()} IceConnectionState: Failed");
                }
                break;
            case RTCIceConnectionState.Max:
                if (printDebug)
                {
                    Debug.Log($"{GetInstanceID()} IceConnectionState: Max");
                }
                break;
            default:
